...
### Activity - Argentina - IPCVA - Precios Internacionales - Precios al Gancho

#creacion_dataset = create_dataset("Activity - Argentina - IPCVA - Precios Internacionales - Precios al Gancho", 1505, "Prueba")
...
#url10 = "http://www.ipcva.com.ar/estadisticas/vista_precios_xls.php?id=2&desde=2004-01-01&hasta={}-{}-{}&categorias=1,2,3,4,5,6,7&paises=1,2,3,4,5,6,7,8,203".format(anio_actual, mes_actual, dia_actual)
...
url10 = parser_base.xpath("//div[@class='span3'][4]//li/a/@href")[1].replace(".php", "_xls.php") + url_parte2

con_gancho = conecting_data(url10, encabezados)
con_gancho = con_gancho[1]
con_gancho = con_gancho.iloc[1:,]
con_gancho.columns = con_gancho.iloc[0]
con_gancho = con_gancho.iloc[1:]
col_dataset_gancho = con_gancho.columns.to_list()
for i, col in enumerate(col_dataset_gancho):
    col = col + "-" + str(con_gancho.iloc[1,i])
    col_dataset_gancho[i] = col
con_gancho.iloc[1] = col_dataset_gancho
con_gancho.columns = con_gancho.iloc[0]
con_gancho = con_gancho.set_index("Fecha")
columns_trans_gancho = con_gancho.columns.unique().tolist()
df_append = []
for pais in columns_trans_gancho:
    df = con_gancho.loc[:,pais]
    df = pd.DataFrame(df)
